...
## install and Load the LAGOtrials package 
```{r}
# install the LAGOtrials package using the tar.gz file
# library(LAGOtrials)

# use the following to load the LAGOtrials library while developing the r package:
devtools::load_all()
```

## load PULESA data and preprocessing for fitting the "main" model 
```{r}
# PULESA data is intentionally NOT pre loaded as part of the LAGOtrials package as the
...
## replicate the optimization results using the LAGOtrials package 
```{r}
...
## replicate the results using the LAGOtrials package 
```{r}
...
#
# ported to the current LAGO API and to the bundled LAGOtrials::main_pulesa_data
...
# [6,]  0.000
# [7,]  1.000
main_pulesa_data <- LAGOtrials::main_pulesa_data
...
optimization_cubic_results <- lago_optimization(
  ...
  cost_list_of_vectors = list(
    c(0, 1700), c(0, 8000), c(0, 9000),
    c(0, 10000), c(0, 11000), c(0, 12000), c(0, 13000) # fake / placeholder costs
  ),
  outcome_goal = 0.8,
