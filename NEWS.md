...
* Fixed `outcome_goal_intention = "minimize"` ignoring the outcome goal on a logit link.
* Fixed the recommended intervention sometimes falling outside the intervention bounds.
* Fixed the numerical optimisation returning its most expensive candidate solution instead of the cheapest.
* Fixed the estimated outcome being wrong when more than one center characteristic was supplied.
* `link = "probit"` and `link = "log"` are no longer accepted, since only `logit` and `identity` were ever implemented.
* Fixed `$est_outcome_ci` sometimes reporting the wrong interval instead of the one at the recommended intervention.
* Fixed the confidence set occasionally dropping one of its qualifying interventions.
* Fixed `confidence_set_size_percentage` understating the size of the confidence set.
* A confidence set containing exactly one grid intervention is no longer discarded as empty.
* Fixed confidence-interval bounds being attached to the wrong interventions under two-way clustering.
* `get_confidence_set()` now returns `rec_int_ci`, the interval at the recommended intervention (its return shape changed for direct callers).
* The confidence set, its size, and the estimated-outcome interval can differ from earlier releases wherever the previous values were wrong.
* Fixed `get_confidence_set()` pairing the prediction matrix with the model coefficients by position instead of by name.
* Fixed the estimated outcome being reported as 0 for the reference period when fixed time effects were included.
* Fixed the estimated-outcome interval being computed at the wrong time period.
* Fixed an additional covariate or center characteristic named like "center" being miscounted as a fixed center effect.
* Fixed a factor or character additional covariate or center characteristic being paired with the wrong coefficient or erroring.
* Fixed the confidence set changing when the rows of `data` were reordered.
* Fixed passing more than one center characteristic to the confidence set failing or duplicating a column.
...
* Added `lago_report()`, which renders a self-contained HTML report of a result.
* Restyled the `print()` and `summary()` console output with boxed `cli` sections.
* The fitted outcome model (`$model`) and the estimated-outcome interval (`$est_outcome_ci`) are now returned on the result.
