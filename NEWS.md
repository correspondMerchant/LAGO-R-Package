...
* The in-browser playground can now hand off to the cost designer: a "Shape costs in the designer" button opens `visualize_cost()` in the browser pre-loaded with the intervention components, bounds and unit costs configured in the playground, so you can shape their cost functions and copy the coefficient list back. The cost designer reads this configuration from its page URL query (`?components=...&lower=...&upper=...&costs=...`) and falls back to its built-in example when the query is absent or malformed; a plain `visualize_cost(...)` call is unchanged.
