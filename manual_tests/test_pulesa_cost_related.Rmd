...
data_tofit<-data.frame(Clinic=original_data$Clinic, Period=original_data$Month, 
...
fit.res<-glm(I(Success/Total_visit) ~ Clinic + Period + AccessMedicines + AccessBPMachines + 
                               HypertensionTraining + DeliveryA + DeliveryB + RemoteMonitoring + 
                               PerformanceImprovement 
                               + MI_DeliveryB + 
                               MI_RemoteMonitoring + MI_PerformanceImprovement, data = data_tofit, 
                             family = "binomial", weight = Total_visit)
fit.res
# from Jingyu's code
# coef for intervention components 
coef_intervention_all<-c(0.02574541,  3.05477149,  0.02858655, -0.01414000, 
                         -0.24840744, -0.01412070, 0.22094251)
# clinic effects in the last period (period 13)
coef_clinic_all<-c(-2.758192, -1.973129, -1.999936, -2.077562, 
...
w_vector_old<-c(avg_n_hypertension_patients, # access to medicines
...
cost_coeff_matri<-matrix(c(0.083, -0.225, 0, 1.3125,
                           0.37, -0.33, 0, 0.1586,
                           ...
                           7.5, -6.13, 0.5, 4.917), byrow = FALSE, nrow = 4)
...
fit.res<-glm(I(Success/Total_visit) ~ Clinic + Period + W1+ W2+ W3+
                     W4+W5+W6+W7+W8+
                     MI_W5+MI_W7+MI_W8, 
                   data = data_tofit,
                   family = "binomial", weight = Total_visit)
